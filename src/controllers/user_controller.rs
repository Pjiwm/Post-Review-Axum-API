...
pub async fn get_by_id(Path(id): Path<u64>) -> impl IntoResponse {
...
pub async fn update(Path(id): Path<u64>, Json(payload): Json<Value>) -> impl IntoResponse {
    let user = user_db::get_by_id(id);
    match &user {
        None => return (StatusCode::NOT_FOUND, Json(json!({"status": "not_found"}))),
        _ => (),
...
pub async fn remove(Path(id): Path<u64>) -> impl IntoResponse {
    let user = user_db::get_by_id(id);
    match &user {
        None => return (StatusCode::NOT_FOUND, Json(json!({"status": "not_found"}))),
        _ => (),
